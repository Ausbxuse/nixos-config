...
This directory is a no-op flake so public builds of `nixos-config` evaluate
...
2. Add the resulting age public key to `nix-secrets/.sops.yaml` under `keys:` and in the relevant `creation_rules`
3. Re-encrypt: `cd nix-secrets && sops updatekeys secrets.yaml`
4. Commit and push `nix-secrets`
