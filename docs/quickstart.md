...
```bash
cd ~/src/public/nixos-config && git add machines/defs.nix && git commit -m "admit NEWHOST"
( cd ~/src/private/nix-secrets && git add .sops.yaml secrets.yaml \
...
```bash
nix run ~/src/public/nixos-config#validate-host
